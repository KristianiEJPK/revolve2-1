...
# Set algorithm, mode, file name, headless, writefiles, writevideos, and genotype_id from command line arguments.
algo = sys.argv[1]
mode = sys.argv[2]
...
genotype_id = int(sys.argv[7])  # New argument for genotype_id
...
def main() -> None:
    ...
    # Load the specific individual from the database using genotype_id.
    ...
    with Session(dbengine) as ses:
        rows = ses.execute(
            ...
            .where(Genotype.id == genotype_id)  # Filter by genotype_id
        ).all()

    if not rows:
        raise ValueError(f"No genotype found with id {genotype_id}")
...
if __name__ == "__main__":
    # run with arguments <algo> <mode> <file_name> <headless> <writefiles> <writevideos> <genotype_id>!!!
